...
class CAE_M(chainer.Chain):
    def __init__(self):
        w = chainer.initializers.HeNormal()
        n_first = 16
        n_latent = 256
        super(CAE_M, self).__init__(
            ce1 = L.Convolution2D(1, n_first, 3, 2, 1, initialW=w),
            ...
            cd1 = L.Deconvolution2D(n_first*4, n_first*2, 3, 2, 1, initialW=w),
            cd2 = L.Deconvolution2D(n_first*2, n_first, 4, 2, 1, initialW=w),
            cd3 = L.Deconvolution2D(n_first, 1, 4, 2, 1, initialW=w),
            d_bn1 = L.BatchNormalization(n_first * 2),
            d_bn2 = L.BatchNormalization(n_first),
            d_bn3 = L.BatchNormalization(1),
        )
